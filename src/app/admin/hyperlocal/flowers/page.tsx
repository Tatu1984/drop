...
import {
  Search, Plus, Eye, RefreshCw, Flower2, DollarSign,
  Package, MapPin, CheckCircle, XCircle, Star, Gift, Boxes
} from 'lucide-react';
...
export default function FlowerStoresPage() {
  const [stores, setStores] = useState<FlowerStore[]>([]);
  const [stats, setStats] = useState<FlowerStats | null>(null);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [selectedStore, setSelectedStore] = useState<FlowerStore | null>(null);
  ...
  const fetchStores = useCallback(async () => {
    try {
      ...
      const params = new URLSearchParams({ type: 'FLOWERS' });
  ...
  return (
    <AdminLayout title="Flowers & Gifts Stores">
      <div className="flex justify-between items-center mb-6">
        <p className="text-gray-500">Manage florists and gift shops</p>
      ...
      <div className="mb-6 p-4 bg-pink-50 border border-pink-200 rounded-lg flex items-start gap-3">
        ...
        <div>
          <h4 className="font-medium text-pink-800">Gifting Category</h4>
          <p className="text-sm text-pink-700">Flower and gift delivery is time-sensitive. Ensure stores can handle same-day and scheduled deliveries.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Card>
          <div className="flex items-center gap-3">
            <div className="p-2 bg-pink-100 rounded-lg"><Flower2 className="h-5 w-5 text-pink-600" /></div>
      ...
      <Card className="mb-6">
        <div className="flex flex-wrap gap-4">
          <div className="flex-1 min-w-[200px]">
            <Input placeholder="Search flower stores..." value={search} onChange={(e) => setSearch(e.target.value)} leftIcon={<Search className="h-5 w-5 text-gray-400" />} />
          </div>
      ...
      {loading ? (
        ...
        <Card><div className="text-center py-12"><Flower2 className="h-12 w-12 text-gray-300 mx-auto mb-4" /><p className="text-gray-500">No flower stores found</p></div></Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {stores.map((store) => (
            <Card key={store.id} padding="none" className="overflow-hidden hover:shadow-lg transition-shadow">
              <div className="relative h-32 bg-gradient-to-br from-pink-100 to-pink-50 flex items-center justify-center">
                <Flower2 className="h-16 w-16 text-pink-300" />
        ...
        {selectedStore && (
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-pink-100 rounded-lg flex items-center justify-center"><Flower2 className="h-8 w-8 text-pink-600" /></div>
        ...
        {selectedStore && (
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div><h3 className="font-semibold">{selectedStore.name}</h3><p className="text-sm text-gray-500">Flower Store Application</p></div>
            </div>
